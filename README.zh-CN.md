...
它自带的是一份为这个项目单独写的 324 词入门词库，让你打开就能用。
...
- **错题当堂解决。** 答错的题在本轮末尾再问一次，答对就明天再见——不会十分钟后又堆到首页的到期数里。
...
- **备份**（首页的 🛟）按设备能给什么就给什么：电脑版 Chrome / Edge 可以**选一个文件夹自动备份**，
  ...
  提醒是按**上次备份之后你又学了多少**来触发的，不只是按天数；新学的词和复习分开算、分开说。
  自动备份永远写两个文件，上一份会留着 —— 覆盖唯一一份副本正是备份可能毁掉它要保护的东西的时刻。
- **从备份恢复** 默认是**合并**（同一个词保留较新的那条），不是替换 ——
