...
function LhomeEntries(){const host=L$("homeEntries");if(!host)return;for(const id of ["learnDrillBtn","learnBrowseBtn","learnWrongBtn","learnMasteredBtn"]){const b=L$(id);if(!b)continue;b.classList.remove("secondary");b.classList.add("entry");
 // Their label starts with an emoji; lifted out so all six entries look alike.
 const t=b.firstChild;if(t&&t.nodeType===3){const m=t.textContent.match(/^\s*(\S+)\s+(.*)$/);if(m){t.textContent=m[2];const ic=document.createElement("span");ic.className="entryIcon";ic.textContent=m[1];b.prepend(ic)}}
 host.appendChild(b)}}
