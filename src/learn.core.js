...
function LvoiceRank(v){const n=`${v.name||""}`;let s=0;
 ...
 return s}
function LvoiceIsGood(v){return !!v&&LvoiceRank(v)>=60}
...
function Lspeak(text){if(!("speechSynthesis" in window)){alert("当前浏览器没有可用的语音朗读功能。");return}
...
function LvoiceAdvice(){const ua=navigator.userAgent||"";
 ...
 if(/Android/i.test(ua))return "用 Chrome 打开可以拿到「Google Deutsch」在线语音，比系统自带的好。";
...
function LrenderVoicePicker(){const sel=L$("voicePick");if(!sel)return;
 ...
 L$("voiceHint").innerHTML=LvoiceIsGood(cur)?"★ 表示高音质语音。"
  :`当前只有基础音质的德语语音。想更自然：${Lesc(LvoiceAdvice())}`}
