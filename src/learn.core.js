...
function Lrender(){const fb=L$("learnFeedback");fb.className="feedback";fb.innerHTML="";L$("learnNextBtn").style.display="none";learnAnswered=false;if(learnPos>=learnQueue.length)return Lfinish();const t=learnQueue[learnPos],c=t.c;L$("learnBadge").textContent=`${t.spot?"已掌握抽查 · ":""}${t.retry?"再来一次 · ":""}${Llabel(t.type)} · ${learnPos+1}/${learnQueue.length}`;L$("learnBar").style.width=`${Math.round(learnPos/learnQueue.length*100)}%`;if(t.type==="intro")Lintro(c);else if(t.type==="recognize")Lrecognize(c);else if(t.type==="reverse")Lreverse(c);else Lspell(c);LcardEnter()}
...
function LcardEnter(){const b=L$("learnBody");if(!b)return;b.classList.remove("enter");void b.offsetWidth;b.classList.add("enter")}
...
function Lrecord(c,ok,type){const s=Lstate(c),t=learnQueue[learnPos]||{},r={ok,again:false,tomorrow:false};
 ...
 Lsave(c,s);return r}
function Lfeedback(c,ok,r={}){const s=Lstate(c),fb=L$("learnFeedback");const next=r.again?" · 本轮末尾再问一次":r.tomorrow?" · 明天再见":(s.due||0)-Date.now()>=12*60*60*1000&&typeof LwhenAgain==="function"?` · 下次${LwhenAgain(s.due)}`:"";fb.className="feedback show "+(ok?"ok":"no");fb.innerHTML=`<b>${ok?"✓ 对了":"✗ 这次先记住它"}</b><div class="answerRow"><div class="deAnswer">${Lesc(c.de)}</div>${LspeakBtn(c.de)}</div><div>${Lesc(Lmeaning(c))} <span class="meta">· ${Lesc(Lenglish(c))}</span></div>${Ldetails(c)}<div class="meta" style="margin-top:8px">当前掌握度：${Math.min(5,s.strength||0)}/5 · 记忆轮次 ${Math.min(3,s.cycles||0)}/3${s.spellingPass?" · 已通过拼写":""}${next}</div><div class="cardLinks">${LeditLink(c)}</div>`}
