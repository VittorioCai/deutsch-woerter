...
function Lintro(c){const edit=LeditLink(c);L$("learnBody").innerHTML=`<div class="wordRow intro"><div class="learnWord">${Lesc(c.de)}</div>${LspeakBtn(c.de)}</div><div class="learnZh">${Lesc(Lmeaning(c))}</div><div class="learnEn">${Lesc(LmeaningMeta(c))}</div>${Ldetails(c)}<div class="learnActions"><button class="primary" id="learnRemember">记住了，下一个</button></div><div class="cardLinks"><button type="button" class="linkBtn" id="learnKnown">这个我已经会</button>${edit?`<span class="dotSep">·</span>${edit}`:""}</div>`;L$("learnRemember").onclick=()=>LintroDone(c,false);L$("learnKnown").onclick=()=>LintroDone(c,true)}
